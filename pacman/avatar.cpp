...
avatar::avatar(int a, int b, bool human, int c) : avatar(a, b, human) {
  color = c;
}
...
bool avatar::parseWordBackward(bool isWord) {
  ...
  bool breakOnSpace = (nextChar != ' ' && curChar != ' ');
  bool breakOnAlpha =
      breakOnSpace && (!isAlpha && !isalnum((unsigned char)nextChar) && isWord);
  bool breakOnNonAlpha = breakOnSpace &&
                         ((isAlpha && isalnum((unsigned char)nextChar)) ||
                          (!isAlpha && isalnum((unsigned char)nextChar))) &&
                         isWord;

  while (true) { // no definite loop #; break when we reach conditions
    if (curChar != ' ' &&
        ((isalnum((unsigned char)nextChar) && breakOnAlpha) ||
         (!isalnum((unsigned char)nextChar) && breakOnNonAlpha))) {
      break;
    } else if (breakOnSpace && nextChar == ' ') {
      break;
    } else if (nextChar == '#') { // not allowed to go on # so break
...
bool avatar::parseWordForward(bool isWord) {
  ...
  while (true) {
    if (curChar != ' ' && (!isalnum((unsigned char)curChar) == !breakOnAlpha) &&
        isWord) { // if they are the same
      break;
