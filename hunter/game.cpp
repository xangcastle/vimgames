...
std::vector<std::string> dictionary = {
    "vim", "code", "exit", "quit", "save", "edit", "bash", "grep", "sed", "awk", "find", "make", "build", "debug",
    "shell", "path", "root", "user"
};
...
void update_game()
{
    ...
    int spawn_rate = 40;
    if (score > 500) spawn_rate = 30;
    if (score > 1000) spawn_rate = 20;
    if (score > 2000) spawn_rate = 10;
...
void handle_input(int ch)
{
    ...
    else if (find_mode)
    {
        ...
        {
            if (enemies[i].active && !enemies[i].is_word && enemies[i].trigger_char == ch)
            {
...
void draw_game()
{
    ...
    // Controls hint
    uiY++;
    draw_text_colored(2, uiY, "[f]<char>: Hunt Ghost   [/]<word>: Hunt Demon   [q]: Give Up", 7);
...
void welcome_screen()
{
    ...
    draw_text_centered_colored(y++, "║                                           ║", 3);
    draw_text_centered_colored(y++, "║         🏹  HUNTER VIM  👻                ║", 6);
    draw_text_centered_colored(y++, "║                                           ║", 3);
    draw_text_centered_colored(y++, "║     Defend the code from bugs & demons!   ║", 3);
    draw_text_centered_colored(y++, "║                                           ║", 3);
    draw_text_centered_colored(y++, "╚═══════════════════════════════════════════╝", 3);
